...
class App extends Component {
	...
	loadWeb3 = async () => {
		// Connecting the application to the blockchain network
		if (window.ethereum) {
	...
	loadBlockchainData = async () => {
		...
		if (daiTokenData) {
			...
			window.alert(
				"Dai token contract not deployed to detected network."
			);
		}
		...
		if (dappTokenData) {
			...
			window.alert(
				"Dapp token contract not deployed to detected network."
			);
		}
